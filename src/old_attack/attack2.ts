...
export async function main(ns: NS): Promise<void> {
	...
	const minWaitBeforeResetMs = 600_000;
	...
	while(true) {
		...
		const attack = new AttackController(ns, hackTarget, availableRam, cores, moneyToTakePercent, 20, 20);
		const timeToPrimeMs = attack.initialPrimeTime();
		...
		while(true) {
			...
			const primingIsFast = timeToPrimeMs < (1000 * 45);
			const primingIsVeryFast = timeToPrimeMs < (1000 * 10);

			if  ((ns.getServerMoneyAvailable(hackTarget) < (ns.getServerMaxMoney(hackTarget) / 4)) ||
					(ns.getServerSecurityLevel(hackTarget) > (ns.getServerBaseSecurityLevel(hackTarget) * 4))) {
			...
			const rebalanceThreshold = primingIsVeryFast ? 4 : primingIsFast ? 10 : 20;
			if (potentiallyUnbalanced > rebalanceThreshold) {
				log(ns, "WARN Server still unbalanced - rebalancing");
				break;
			}
